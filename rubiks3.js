...
function findFaceFromMesh(cube, xDir, yDir, normal) {
...
function findFaceFromCubeList() {
    const foundFaces = faceIndexes.filter((faceIndex) => {
        const matchingCubes = gestureData.cubeList.filter((cube) => {
            return isCubeInFace(cube, faceIndex);
        });

        return matchingCubes.length == gestureData.cubeList.length;
    });

    console.log(`cubeList:`, gestureData.cubeList);
    console.log(`foundFaces:`, foundFaces);

    if (foundFaces.length > 0) {
        return foundFaces[0].name;
    }
}

function getCubeIntersect(pointX, pointY) {
    const pointer = new THREE.Vector2();
    const raycaster = new THREE.Raycaster();
    pointer.x = ( pointX / window.innerWidth ) * 2 - 1;
    pointer.y = - ( pointY / window.innerHeight ) * 2 + 1;
    raycaster.setFromCamera( pointer, camera );
    const intersects = raycaster.intersectObjects( cubes );
    if (intersects.length > 0) {
        let cube = intersects[0].object;
        const intNorm = intersects[0].normal.applyEuler( cube.rotation );
        return { cube: cube, normal: intNorm };
    }

    return { cube: null, normal: null };
}
...
function findFaceFromGesture() {
    ...
    if (xDir != 0 || yDir != 0) {
        ...
        if (intersects.length == 0) {
            ...
            isRotating = true;
            const intNorm = intersects[0].normal.applyEuler( intersects[0].object.rotation);
            console.log(`normal`, intNorm);
            ...
            //} else {
                ...
                //faceToRotate = findFaceFromMesh(intersects[0].object, xDir, yDir, intNorm);
                faceToRotate = findFaceFromCubeList();
                isRotating = true;
...
function handleMoveStart(x, y) {
    if (isInteractive) {
        ...
        // TODO: Okay we need to make this figure out what you clicked on, and I guess set that in gestureData?
        // It also needs to reliably figure out which normal was clicked.
        // Why were the normal scrambled before but seemingly not now? Maybe put some debug stuff here at moveStart.
        // Resurrect onmove, and that is where we will need to figure out the axis somehow.
        // Then, once you get that direction, lock it into gestureData as well so we aren't calculating axis on every
        // frame.
    }
}
...
function handleMove(x, y) {
    ...
    const { startCube, cubeInt } = getCubeIntersect(x, y);
    if (startCube && gestureData.cubeList.filter((p) => veq(p.position, startCube.position)).length == 0) {
        gestureData.cubeList.push(startCube);
    }
